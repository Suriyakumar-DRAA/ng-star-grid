import { Component, EventEmitter, Input, Output, OnInit, OnDestroy, OnChanges, ViewEncapsulation } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { Subject, takeUntil } from 'rxjs';
import { FilterOption, Employee, FilterPanelApplyEvent } from '../../interfaces/data-table.interface';
...
export class FilterPanelComponent implements OnInit, OnDestroy, OnChanges {
  ...
  private loadFilterOptions(): void {
    ...
    this.loading = true;
    this.dataService.getFilterOptions(this.column as keyof Employee)
      .pipe(takeUntil(this.destroy$))
      .subscribe(options => {
  ...
  private checkIfTextColumn(): void {
    ...
    const textColumns: (keyof Employee)[] = ['name', 'email', 'department', 'location'];
    const numberColumns: (keyof Employee)[] = ['id', 'salary'];
    const dateColumns: (keyof Employee)[] = ['joinDate'];
  ...
  get isTextColumn(): boolean {
    const textColumns: (keyof Employee)[] = ['name', 'email', 'department', 'location'];
    return textColumns.includes(this.column);
  }

  get isNumberColumn(): boolean {
    const numberColumns: (keyof Employee)[] = ['id', 'salary'];
    return numberColumns.includes(this.column);
  }

  get isDateColumn(): boolean {
    const dateColumns: (keyof Employee)[] = ['joinDate'];
    return dateColumns.includes(this.column);
  }
